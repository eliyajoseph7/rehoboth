...
class DailyReportController extends Controller
{
    public function generate()
    {
        ...
        for($i = 1; $i <= $total_days; $i++) {
            ...
            foreach ($takens as $taken) {
                $exist = DailyReport::where(DB::raw("(DATE_FORMAT(date,'%Y-%m-%d'))"), $taken->date)->first();
                if ($exist) {
            ...
            foreach ($returns as $return) {
                $exist = DailyReport::where(DB::raw("(DATE_FORMAT(date,'%Y-%m-%d'))"), $return->date)->first();
                if ($exist) {
            ...
            foreach ($allowances as $allowance) {
                $exist = DailyReport::where(DB::raw("(DATE_FORMAT(date,'%Y-%m-%d'))"), $allowance->date)->first();
                if ($exist) {
            ...
            foreach ($expenditures as $expenditure) {
                $exist = DailyReport::where(DB::raw("(DATE_FORMAT(date,'%Y-%m-%d'))"), $expenditure->date)->first();
                if ($exist) {
            ...
            foreach ($supports as $support) {
                $exist = DailyReport::where(DB::raw("(DATE_FORMAT(date,'%Y-%m-%d'))"), $support->date)->first();
                if ($exist) {
            ...
            if(count($takens) == 0 && count($returns) == 0 && count($allowances) == 0 && count($expenditures) == 0 && count($supports) == 0) {
                $newreport = DailyReport::where(DB::raw("(DATE_FORMAT(date,'%Y-%m-%d'))"), now()->format('Y-m-d'))->first();
                if(!$newreport) {
                    $report->save();
                }
    ...
    {
        $totals = DailyReport::whereMonth('date', $month)->whereYear('date', $year)
            ->select(
