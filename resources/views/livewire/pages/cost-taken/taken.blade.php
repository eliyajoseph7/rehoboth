    ...
    <div>
            ...
            <div class="max-w-full mx-auto sm:px-6 lg:px-0">
                <div class="w-full">
                    <div class="flex justify-end pb-2">
                        <button wire:click="$dispatch('openModal', {component: 'pages.cost-taken.taken-form'})"
                                            ...
                                            @forelse ($data as $dt)
                                                <tr wire:key="{{ $dt->id }}"
                                                    ...
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        {{ $dt->code }}</td>
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        {{ $dt->paymentMethod?->name }}</td>
                                                    <td class="px-4 py-3 flex items-center justify-end space-x-1">
                                                        <button title="Update"
                                                            wire:click="$dispatch('openModal', {component: 'pages.cost-taken.taken-form', arguments: {id: {{ $dt->id }}}})"
        ...
        <script data-navigate-once>
            document.addEventListener('livewire:init', () => {
                ...
                Livewire.on('confirm_delete', (id) => {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            Livewire.dispatch('delete_cost_taken', {
                                'id': id
                            });
                        }
                    });
                ...
                Livewire.on('cost_taken_deleted', () => {
                    Toast.fire({
                        icon: 'success',
                ...
                // Livewire.on('edit_cost_taken', (id) => {
                //     Livewire.dispatch('update_cost_taken', {'id': id});
                // })
        ...
        <script data-navigate-once>
            $(document).on('change', '#payment_method_id', function(e) {
                ...
                @this.set('payment_method_id', e.target.value);
                Livewire.dispatch('set_payment_method_id', {
                    'id': e.target.value
                });
            });
