    ...
    <div>
            ...
            <div class="max-w-full mx-auto sm:px-6 lg:px-0">
                <div class="w-full">
                    <div class="flex justify-end pb-2">
                        <button wire:click="$dispatch('openModal', {component: 'pages.support.support-form'})"
                            ...
                            <span class="">{{ __('Add Support') }}</span>
                        </button>
                                            ...
                                            @forelse ($data as $dt)
                                                <tr wire:key="{{ $dt->id }}"
                                                    ...
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        {{ $dt->date }}</td>
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        {{ $dt->supportCategory?->name }}</td>
                                                    <td class="px-4 py-3 whitespace-nowrap">
                                                        {{ $dt->amount }}</td>
                                                    <td class="px-4 py-3 flex items-center justify-end space-x-1">
                                                        <button title="Update"
                                                            wire:click="$dispatch('openModal', {component: 'pages.support.support-form', arguments: {id: {{ $dt->id }}}})"
        ...
        <script data-navigate-once>
            document.addEventListener('livewire:init', () => {
                ...
                Livewire.on('confirm_delete', (id) => {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            Livewire.dispatch('delete_support', {
                                'id': id
                            });
                        }
                    });
                ...
                // Livewire.on('edit_support', (id) => {
                //     Livewire.dispatch('update_support', {'id': id});
                // })

            })
        </script>
        <script data-navigate-once>
            $(document).on('change', '#support_category_id', function(e) {
                console.log(e.target.value);
                @this.set('support_category_id', e.target.value);
                Livewire.dispatch('set_support_category_id', {
                    'id': e.target.value
                });
            });
            ...
            document.addEventListener('livewire:init', () => {
                ...
                Livewire.on('reset_support_category_id', (e) => {
                    $('#support_category_id').val('').trigger('change')
                });

                Livewire.on('update_support_category_id_field', (data) => {
                    $('#support_category_id').val(data).trigger('change')
                });
            });
