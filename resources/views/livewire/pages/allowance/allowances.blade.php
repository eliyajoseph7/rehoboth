    ...
    <div>
            ...
            <div class="max-w-full mx-auto sm:px-6 lg:px-0">
                <div class="w-full">
                    <div class="flex justify-end pb-2">
                        <button wire:click="$dispatch('openModal', {component: 'pages.allowance.allowance-form'})"
                            ...
                            <span class="">{{ __('Add Allowance') }}</span>
                        </button>
                                            ...
                                            @forelse ($data as $dt)
                                                <tr wire:key="{{ $dt->id }}"
                                                    ...
                                                    <td class="px-4 py-3 flex items-center justify-end space-x-1">
                                                        <button title="Update"
                                                            wire:click="$dispatch('openModal', {component: 'pages.allowance.allowance-form', arguments: {id: {{ $dt->id }}}})"
        ...
        <script data-navigate-once>
            document.addEventListener('livewire:init', () => {
                ...
                Livewire.on('confirm_delete', (id) => {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            Livewire.dispatch('delete_allowance', {
                                'id': id
                            });
                        }
                    });
                ...
                //     Livewire.dispatch('update_allowance', {'id': id});
                // })
        ...
        <script data-navigate-once>
            $(document).on('change', '#staff_id', function(e) {
                ...
                @this.set('staff_id', e.target.value);
                Livewire.dispatch('set_staff_id', {
                    'id': e.target.value
                });
            });
            ...
            document.addEventListener('livewire:init', () => {
                ...
                Livewire.on('reset_staff_id', (e) => {
                    $('#staff_id').val('').trigger('change')
                });

                Livewire.on('update_staff_id_field', (data) => {
                    $('#staff_id').val(data).trigger('change')
                });
            });
